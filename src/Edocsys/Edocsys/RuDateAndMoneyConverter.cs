...
// Конвертер в строку для чисел прописью (в именительном и родительном падежах) (включая деньги в рублях), а также для дат 
...
namespace Edocsys
{
    ...
    {
        static string[] monthNamesGenitive = { "", "января", "февраля", "марта", "апреля", "мая",
        ...
        /// <param name="_firstCapital"></param>
        /// <returns></returns>
        public static string CurrencyToTxtFull_SBR(double _amount, bool _firstCapital)
        {
            //10 000 (Десять тысяч) рублей 67 копеек
            long rublesAmount = (long)Math.Floor(_amount);
            long copecksAmount = ((long)Math.Round(_amount * 100)) % 100;
            ...
            string s = String.Format("{1} ({0:N0}) ", rublesAmount, NumeralsToTxt(rublesAmount, TextCase.Nominative, true, _firstCapital));
